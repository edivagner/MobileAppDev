...
function testNum(num1: Number, num2: Number): string {
...
function evaluateNum(): void {
    let updateText = "";
    // const input: HTMLInputElement | null
    const input1 = document.getElementById('firstNumber') as HTMLInputElement | null;
    const input2 = document.getElementById('secondNumber') as HTMLInputElement | null;

    if ((input1 || input2) != null) {
        // input has type HTMLInputElement here
        console.log(input1?.value); // "Initial value"
        console.log(input2?.value); // "Initial value"

        updateText = testNum(Number(input1?.value), Number(input2?.value));
    }
